...
@Service
public class PlayMatchRpsImp  implements PlayMatch<PlayRps, MatchRps> {
    @Autowired
    ...
    private void evaluatePlay(PlayRps playRps, MatchRps matchRps){
        if(!isPlayTie(playRps)){
            setWinner(playRps, matchRps); // donde seteo wl winner??
        }
    }
    ...
    private Boolean isMatchTie(MatchRps matchRps) {
        return matchRps.getPlayer1Score().equals(matchRps.getPlayer2Score());// porque usa el equals y no == si compara numeros??
    }
